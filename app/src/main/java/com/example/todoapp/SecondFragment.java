...
public class SecondFragment extends Fragment {
    ...
    private ChiTietNhanService chiTietNhanService;
    @Override
                        ...
                        if (result.getResultCode() == RESULT_OK) {
                            ...
                            if (result.getData() != null) {
                                ...
                                if (newNote != null) {
                                    ...
                                    SecondFragment secondFragment = (SecondFragment) getParentFragmentManager().findFragmentById(R.id.mainFrame);
                                    if (secondFragment != null) {
                                        ArrayList a = (ArrayList) arrayList;
                                        secondFragment.editNote(newNote,(result.getData().getIntExtra("position",-1)));
                                    }
                                }
                            }
                        }
...
//                adapter.notifyItemChanged(position);
    ...
    public void editNote(GhiChu newNote, int position) {
        if (newNote != null) {
            // Check if newNote contains the specific query
            if (queryWithWildcard != null && queryWithWildcard.length() >= 2) {
                // Remove the first character
                queryWithWildcard = queryWithWildcard.substring(1);
                // Remove the last character
                queryWithWildcard = queryWithWildcard.substring(0, queryWithWildcard.length() - 1);

                if (!searchByTag) {
                    if (!containsQuery(newNote, queryWithWildcard)) {
                        Toast.makeText(getContext(), queryWithWildcard, Toast.LENGTH_SHORT).show();
    ...
    private boolean containsQuery(GhiChu ghiChu, String query) {
        // Adjust this logic based on your GhiChu class structure
        return ghiChu != null &&
                (ghiChu.getTieuDe() != null && ghiChu.getTieuDe().contains(query)) ||
                (ghiChu.getNoiDung() != null && ghiChu.getNoiDung().contains(query));
    }
    ...
    public void search(String taiKhoanGhiNho, String query){
        queryWithWildcard = "%" + query + "%";
        arrayList.clear();
        arrayList.addAll(ghiChuService.timKiemGhiChu(taiKhoanGhiNho, query, queryWithWildcard));
        adapter.notifyDataSetChanged();
    }
    public void searchByTag(int maNhan){
        searchByTag = true;
        arrayList.clear();
        arrayList.addAll(chiTietNhanService.getListGhiChuOfNhan(maNhan));
        adapter.notifyDataSetChanged();
    }
}